...
class Evaluator(object):
    ...
    def load_data(self):
        ...
        self.passenger_id = np.load(os.path.join(self.x_home, 'passenger_id.npy'))
        assert len(self.x) == len(self.y) == len(self.t) == len(self.passenger_id), 'Length of x, y, t and filename are different.'


    def evaluate(self):
        logger.info('Evaluating...')
        scores = pd.DataFrame(columns=['PassengerID', 'Predict', 'Actual'])

        logger.info('Saving result images...')
        for y, t, passenger_id in zip(self.y, self.t, self.passenger_id):
            # Store with DataFrame
            idx = len(scores.index)
            scores.loc[idx, 'PassengerID'] = passenger_id
            scores.loc[idx, 'Predict'] = y[0]
            scores.loc[idx, 'Actual'] = t[0]
